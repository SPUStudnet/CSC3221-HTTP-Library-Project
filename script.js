...
function showError(err) {
  const html = `<p style='color: red;'>${err}</p>`;
  document.querySelector("#response").innerHTML = html;
}

async function sendRequest(reqType, targetURL, data) {
  try {
    const response = await http[reqType](targetURL, data);
    showResponse(response);
  } catch (error) {
    showError('Failed to execute request: ' + error);
  }
}
...
function setupRequest(e) {
  ...
  const userId = document.querySelector("#uIdArea>input").value;
  const fullName = document.querySelector("#uNameArea>input").value;
  ...
  if (reqType === 'get' || reqType === 'delete') {
    url += userId; // Append user ID to URL for GET and DELETE requests
    ...
    if (fullName) {
      ...
      data = {
        name: fullName,
        username: firstName,
        email: `${firstName.toLowerCase()}@spu.edu`
      };
    }
  }

  sendRequest(reqType.toLowerCase(), url, data);
}

// Attach event listener to form submission
document.querySelector("#SendReq").addEventListener("click", setupRequest);

// Initial setup for input fields based on the selected HTTP method
function setupInputFields() {
  const method = document.querySelector("input[name='HTTPtype']:checked").value;
  document.querySelector("#uIdArea").style.display = (method === 'get' || method === 'delete') ? 'flex' : 'none';
  document.querySelector("#uNameArea").style.display = (method === 'post' || method === 'put') ? 'flex' : 'none';
}

// Setup event listeners for radio buttons to adjust input fields
document.querySelectorAll("input[name='HTTPtype']").forEach(radioButton => {
  radioButton.addEventListener('change', setupInputFields);
});

setupInputFields(); // Run on initial load
